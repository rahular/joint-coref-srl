...
class SpanConstituencyParser(Model):
    ...
    def forward(
        ...
        if (
            all(batch_gold_trees)
            and self._evalb_score is not None
    ...
    def construct_trees(
        ...
        for batch_index, (scored_spans, spans, sentence) in enumerate(
            ...
            for prediction, span in zip(
                ...
                if int(label_index) != no_label_id or (
                    start == 0 and end == len(sentence)
                ):
            ...
            spans_to_labels = {
                (span.start, span.end): self.vocab.get_token_from_index(
            ...
            trees.append(
                self.construct_tree_from_spans(spans_to_labels, sentence, sentence_pos)
            )
