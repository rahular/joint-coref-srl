...
class Elmo(Subcommand):
    ...
    def add_subparser(
        ...
        subparser.add_argument(
            "output_file", type=str, help="The path to the output file."
        )

        group = subparser.add_mutually_exclusive_group(required=True)
        group.add_argument(
            "--all", action="store_true", help="Output all three ELMo vectors."
        )
        group.add_argument(
            "--top", action="store_true", help="Output the top ELMo vector."
        )
        group.add_argument(
            "--average",
            action="store_true",
            help="Output the average of the ELMo vectors.",
        )
...
class ElmoEmbedder:
            ...
            for i in range(len(batch)):
                ...
                if length == 0:
                    ...
                    elmo_embeddings.append(
                        embeddings[i, :, :length, :].detach().cpu().numpy()
                    )

        return elmo_embeddings

    def embed_sentences(
        self, sentences: Iterable[List[str]], batch_size: int = DEFAULT_BATCH_SIZE
    ...
    def embed_file(
        ...
        if use_sentence_keys:
            ...
            embedded_sentences = (
                (str(i), x)
                for i, x in enumerate(self.embed_sentences(split_sentences, batch_size))
            )
        ...
        with h5py.File(output_file_path, "w") as fout:
            for key, embeddings in Tqdm.tqdm(embedded_sentences):
                ...
                fout.create_dataset(
                    str(key), output.shape, dtype="float32", data=output
                )
            if not forget_sentences and not use_sentence_keys:
